 ...
 *
 *   This is a factory class used to manage the output port. It creates and
 * deletes the output channel functionality as needed to support any new
 * configurations that get sent from from the WebPage.
 *
 */
...
class c_OutputCommon; ///< forward declaration to the pure virtual output class
...
#define OM_MAX_NUM_CHANNELS (4500 * 3)
...
public:
  ...
  void Begin(); ///< set up the operating environment based on the current
                ///< config (or defaults)
  ...
  void SetConfig(
      const char *NewConfig); ///< Save the current configuration data to nvram
  void
  SetConfig(ArduinoJson::JsonDocument
                &NewConfig); ///< Save the current configuration data to nvram
  void GetStatus(JsonObject &jsonStatus);
  //    void      GetPortCounts     (uint16_t& PixelCount, uint16_t&
  //    SerialCount) {PixelCount = uint16_t(OutputPortId_End); SerialCount =
  //    uint16_t(NUM_UARTS); }
  uint8_t *GetBufferAddress() {
    return pOutputBuffer;
  } ///< Get the address of the buffer into which the E1.31 handler will stuff
    ///< data
  uint32_t GetBufferUsedSize() {
    return UsedBufferSize;
  } ///< Get the size (in intensities) of the buffer into which the E1.31
    ///< handler will stuff data
  uint32_t GetBufferSize() {
    return uint32_t(OM_MAX_NUM_CHANNELS);
  } ///< Get the size (in intensities) of the buffer into which the E1.31
    ///< handler will stuff data
  ...
  void WriteChannelData(uint32_t StartChannelId, uint32_t ChannelCount,
                        uint8_t *pData);
  void ReadChannelData(uint32_t StartChannelId, uint32_t ChannelCount,
                       uint8_t *pTargetData);
  void ClearBuffer();
  void TaskPoll();
  ...
  // do NOT insert into the middle of this list. Always add new types to the end
  // of the list
...
#define NO_CONFIG_NEEDED time_t(-1)
  ...
  void SetSerialUart();
  bool FindJsonChannelConfig(JsonDocument &jsonConfig, OM_PortId_t PortId,
                             e_OutputProtocolType Type, JsonObject &ChanConfig);
  bool SetPortDefnitionDefaults(DriverInfo_t &CurrentOutput,
