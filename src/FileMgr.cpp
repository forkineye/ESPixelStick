...
//-----------------------------------------------------------------------------
void ftp_callback(FtpOperation ftpOperation, unsigned int freeSpace,
...
//-----------------------------------------------------------------------------
void ftp_transferCallback(FtpTransferOperation ftpOperation, const char *name,
  ...
  case FTP_UPLOAD: {
    ...
    // transferredSize); size_t free = heap_caps_get_largest_free_block(0x1800);
    // LOG_PORT.printf("heap: 0x%x\n", free);
    break;
  }
...
void c_FileMgr::Begin() {
  ...
  do // once
  {
...
#endif
    {
      String msg = String(CN_stars) +
                   F(" Flash file system did not initialize correctly ") +
                   CN_stars;
    ...
    logcon(String(F("Flash file system initialized. Used = ")) +
           String(LittleFS.usedBytes()) + String(F(" out of ")) +
...
void c_FileMgr::SetSpiIoPins() {
...
#endif // def ARDUINO_ARCH_ESP32
  {
    ...
    // on some hardware MISO is missing a required pull-up resistor, use
    // internal pull-up.
...
#endif // !def SUPPORT_SD_MMC
    {
      ...
      if (nullptr == ESP_SD.card()) {
        ...
        logcon(String(F("SD Card Error: initialization failed - code: ")) +
               String(ESP_SD.card()->errorCode()));
        // DEBUG_V(String(F("SD Card initialization failed - data: ")) +
        // String(ESP_SD.card()->errorData())); printSdErrorText(&LOG_PORT,
        // ESP_SD.card()->errorCode()); LOG_PORT.println("");
...
void c_FileMgr::SetSdSpeed() {
  ...
  if (SdCardInstalled) {
    ...
    // DEBUG_V(String("TRAN Speed: 0x") +
    // String(MyCsd.Decode_0.tran_speed,HEX));
...
void c_FileMgr::RenameFlashFile(String OldName, String NewName) {
  ...
  if (!LittleFS.rename(OldName, NewName)) {
    logcon(String(CN_stars) + F("Could not rename '") + OldName + F("' to '") +
           NewName + F("'") + CN_stars);
  }
...
void c_FileMgr::listDir(fs::FS &fs, String dirname, uint8_t levels) {
  ...
  do // once
  {
    ...
    if (!root) {
      logcon(String(CN_stars) + F("failed to open directory: ") + dirname +
             CN_stars);
      break;
    }
...
bool c_FileMgr::LoadFlashFile(const String &FileName,
  ...
  do // once
  {
    String CfgFileMessagePrefix =
        String(CN_Configuration_File_colon) + "'" + FileName + "' ";
    ...
    /*
            ...
            {
                logcon (String(CN_stars) + CfgFileMessagePrefix + F ("Could not
       read file.") + CN_stars); break;
            }
            logcon(RawFileData);
    */
    // DEBUG_V();
    ...
    // DEBUG_V();
    if (!file) {
      logcon(String(CN_stars) + CfgFileMessagePrefix +
             String(F(" Could not open file for reading ")) + CN_stars);
      break;
    }
    ...
    if (error) {
      ...
      logcon(String(CN_stars) + CfgFileMessagePrefix +
             String(F("Deserialzation Error. Error code = ")) + error.c_str() +
             CN_stars);
      // logcon (CN_plussigns + RawFileData + CN_minussigns);
      // DEBUG_V (String ("                heap: ") + String (ESP.getFreeHeap
      // ()));
      //  xDEBUG_V (String (" getMaxFreeBlockSize: ") + String
...
bool c_FileMgr::SaveFlashFile(const String &FileName, const char *FileData) {
  ...
  if (!file) {
    logcon(String(CN_stars) + CfgFileMessagePrefix +
           String(F("Could not open file for writing..")) + CN_stars);
  } else {
    file.seek(0, SeekSet);
    file.print(FileData);
    file.close();
    ...
    logcon(CfgFileMessagePrefix + String(F("saved ")) + String(file.size()) +
           F(" bytes."));
    file.close();
...
bool c_FileMgr::SaveFlashFile(const String &FileName, JsonDocument &FileData) {
  ...
  if (!file) {
    ...
    logcon(CfgFileMessagePrefix + String(F("saved ")) + String(NumBytesSaved) +
           F(" bytes."));
...
//-----------------------------------------------------------------------------
bool c_FileMgr::SaveFlashFile(const String &FileName, uint32_t index,
                              uint8_t *data, uint32_t len, bool final) {
  // DEBUG_START;
  bool Response = false;
...
bool c_FileMgr::ReadFlashFile(const String &FileName, String &FileData) {
  ...
  if (file) {
    ...
    // logcon (CfgFileMessagePrefix + String (F ("reading ")) + String
    // (file.size()) + F (" bytes."));
    ...
    logcon(String(CN_stars) + CN_Configuration_File_colon + "'" + FileName +
           F("' not found.") + CN_stars);
  }
...
bool c_FileMgr::ReadFlashFile(const String &FileName, JsonDocument &FileData) {
  ...
  do // once
  {
    ...
    DeserializationError error =
        deserializeJson(FileData, (const String)RawFileData);
...
//-----------------------------------------------------------------------------
bool c_FileMgr::ReadFlashFile(const String &FileName, byte *FileData,
                              size_t maxlen) {
  // DEBUG_START;
  bool GotFileData = false;
  ...
  do // once
  {
    ...
    if (file.size() >= maxlen) {
      logcon(String(CN_stars) + CN_Configuration_File_colon + "'" + FileName +
             F("' too large for buffer. ") + CN_stars);
      file.close();
      break;
    }
...
int c_FileMgr::FileListFindSdFileHandle(FileId HandleToFind) {
  ...
  if (INVALID_FILE_HANDLE != HandleToFind) {
    for (auto &currentFileListEntry : FileList) {
      // DEBUG_V (String ("currentFileListEntry.handle: ")  + String
      // (currentFileListEntry.handle)); DEBUG_V (String
      // ("currentFileListEntry.entryId: ") + String
      // (currentFileListEntry.entryId));
...
c_FileMgr::FileId c_FileMgr::CreateSdFileHandle() {
  ...
  if (INVALID_FILE_HANDLE == response) {
    logcon(String(CN_stars) + F(" Could not allocate another file handle ") +
           CN_stars);
  }
...
void c_FileMgr::GetListOfSdFiles(std::vector<String> &Response) {
  ...
  do // once
  {
    ...
    if (!root) {
      logcon(String(CN_stars) + F("failed to open directory: ") + CN_slashsd +
             CN_stars);
      break;
    }
    ...
    while (MyFile) {
      ...
      if ((!EntryName.isEmpty()) && (0 != MyFile.size())) {
...
void c_FileMgr::SaveSdFile(const String &FileName, String &FileData) {
  ...
  do // once
  {
    ...
    int WriteCount = WriteSdFile(FileHandle, (byte *)FileData.c_str(),
...
//-----------------------------------------------------------------------------
bool c_FileMgr::OpenSdFile(const String &_FileName, FileMode Mode,
                           FileId &FileHandle, int FileListIndex) {
  // DEBUG_START;
  ...
  do // once
  {
    ...
    if (FileMode::FileRead == Mode) {
      ...
      if (false == ESP_SD.exists(FileName)) {
        logcon(String(F("ERROR: Cannot find '")) + FileName +
               F("' for reading. File does not exist."));
        break;
      }
    }
    // DEBUG_V ("File Exists");
    ...
    if (-1 == FileListIndex) {
      ...
      // DEBUG_V(String("Using predefined File Index: ") +
    ...
    if (-1 != FileListIndex) {
      ...
      LockSd();
#ifdef SIMULATE_SD
      FileList[FileListIndex].fsFile =
          ESP_SDFS.open(FileName, &XlateFileMode[Mode]);
      FileList[FileListIndex].IsOpen = true;
#else
      FileList[FileListIndex].IsOpen = FileList[FileListIndex].fsFile.open(
          FileList[FileListIndex].Filename.c_str(), XlateFileMode[Mode]);
#endif // def SIMULATE_SD
      UnLockSd();

      // DEBUG_V(String("ReadWrite: ") + String(XlateFileMode[Mode]));
      // xDEBUG_V(String("File Size: ") +
      // String64(FileList[FileListIndex].fsFile.size()));
      ...
      UnLockSd();
      // DEBUG_V(String(FileList[FileListIndex].Filename) + " - " +
      // String(FileList[FileListIndex].size));
      ...
      FileIsOpen = true;
      // DEBUG_V (String ("File.Handle: ") + String
  ...
  if (true == OpenSdFile(FileName, FileMode::FileRead, FileHandle, -1)) {
    ...
    if (-1 != (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
      ...
      if (error) {
        ...
        logcon(CN_Heap_colon + String(ESP.getFreeHeap()));
        logcon(CfgFileMessagePrefix +
               String(F("Deserialzation Error. Error code = ")) +
               error.c_str());
        logcon(CN_plussigns + RawFileData + CN_minussigns);
      } else {
        GotFileData = true;
...
//-----------------------------------------------------------------------------
uint64_t c_FileMgr::ReadSdFile(const FileId &FileHandle, byte *FileData,
  ...
  if (-1 != (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
    uint64_t BytesRemaining =
        uint64_t(FileList[FileListIndex].size - StartingPosition);
...
#ifdef SIMULATE_SD
    response = FileList[FileListIndex].fsFile.readBytes((char *)FileData,
                                                        ActualBytesToRead);
#else
    response =
        FileList[FileListIndex].fsFile.readBytes(FileData, ActualBytesToRead);
#endif // def SIMULATE_SD
    UnLockSd();
    // DEBUG_V(String("         response: ") + String64(response));
  } else {
    logcon(String(F("ReadSdFile::ERROR::Invalid File Handle: ")) +
           String(FileHandle));
  }
...
void c_FileMgr::CloseSdFile(FileId &FileHandle) {
  ...
  if (-1 != (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
    ...
    if (nullptr != FileList[FileListIndex].buffer.DataBuffer) {
      if (FileList[FileListIndex].buffer.DataBuffer !=
    ...
    logcon(String(F("CloseSdFile::ERROR::Invalid File Handle: ")) +
           String(FileHandle));
  }
...
//-----------------------------------------------------------------------------
uint64_t c_FileMgr::WriteSdFile(const FileId &FileHandle, byte *FileData,
  ...
  do // once
  {
    ...
    if (-1 == (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
      // DEBUG_V (String("FileHandle: ") + String(FileHandle));
      logcon(String(F("WriteSdFile::ERROR::Invalid File Handle: ")) +
             String(FileHandle));
      break;
    }
    ...
    LockSd();
    // DEBUG_V();
    NumBytesWritten = FileList[FileListIndex].fsFile.write((uint8_t *)FileData,
                                                           NumBytesToWrite);
    // DEBUG_V();
    ...
    if (NumBytesWritten != NumBytesToWrite) {
      logcon(String(F("ERROR: SD Write failed. Tried writting ")) +
             String(NumBytesToWrite) + F(" bytes. Actually wrote ") +
             String(NumBytesWritten) + F(" bytes."));
      NumBytesWritten = 0;
      break;
    }
  } while (false);
...
*/
uint64_t c_FileMgr::WriteSdFileBuf(const FileId &FileHandle, byte *FileData,
                                   uint64_t NumBytesInSourceBuffer) {
  // DEBUG_START;
  ...
  {
    int FileListIndex;
    // DEBUG_V (String("    NumBytesInSourceBuffer: ") +
    // String(NumBytesInSourceBuffer)); DEBUG_V (String(" ForceWriteToSD: ") +
    // String(ForceWriteToSD));
    if (-1 == (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
      logcon(String(F("WriteSdFileBuf::ERROR::Invalid File Handle: ")) +
             String(FileHandle));
      break;
    }
    ...
    if (nullptr == FileList[FileListIndex].buffer.DataBuffer) {
      ...
      LockSd();
      NumBytesWrittenToDestBuffer = FileList[FileListIndex].fsFile.write(
          FileData, NumBytesInSourceBuffer);
      FileList[FileListIndex].fsFile.flush();
      UnLockSd();
    ...
    } else // buffered mode
    {
      ...
      // DEBUG_V(String("     NumBytesInSourceBuffer: ") +
      // String(NumBytesInSourceBuffer));
      uint64_t SpaceRemaining = FileList[FileListIndex].buffer.size -
                                FileList[FileListIndex].buffer.offset;
      // DEBUG_V(String("             SpaceRemaining: ") +
      // String(SpaceRemaining));

      if (ForceWriteToSD || (NumBytesInSourceBuffer &&
                             (SpaceRemaining < NumBytesInSourceBuffer))) {
        // DEBUG_V("Write out the buffer");
        // delay(20);
        // DEBUG_V(String("       BytesToBeWrittenToSD: ") +
        // String(FileList[FileListIndex].buffer.offset));
        FeedWDT();
        LockSd();
        uint64_t WroteToSdSize = FileList[FileListIndex].fsFile.write(
        ...
        delay(30);
        FeedWDT();
        // DEBUG_V(String("                     offset: ") +
        // String(FileList[FileListIndex].buffer.offset)); DEBUG_V(String("
        // WroteToSdSize: ") + String(WroteToSdSize));
        if (FileList[FileListIndex].buffer.offset != WroteToSdSize) {
          logcon(
              String("WriteSdFileBuf:ERROR:SD Write Failed. Tried to write: ") +
              String(FileList[FileListIndex].buffer.offset) +
        ...
        // reset the buffer
        FileList[FileListIndex].buffer.offset = 0;
        // DEBUG_V(String("                 New offset: ") +
        // String(FileList[FileListIndex].buffer.offset));
        ForceWriteToSD = false;
      }

      if (NumBytesInSourceBuffer) {
        // DEBUG_V(String("                    Writing ") +
        // String(NumBytesInSourceBuffer) + " bytes to the buffer");
        // DEBUG_V(String("                     offset: ") +
        // String(FileList[FileListIndex].buffer.offset));
        memcpy(&(FileList[FileListIndex]
                     .buffer.DataBuffer[FileList[FileListIndex].buffer.offset]),
               FileData, NumBytesInSourceBuffer);
        ...
        // String(NumBytesWrittenToDestBuffer)); DEBUG_V(String("
        // NumBytesInSourceBuffer: ") + String(NumBytesInSourceBuffer));
        // DEBUG_V(String("                 New offset: ") +
        // String(FileList[FileListIndex].buffer.offset));
...
uint64_t c_FileMgr::GetSdFileSize(const FileId &FileHandle) {
  ...
  if (-1 != (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
    ...
    logcon(String(F("GetSdFileSize::ERROR::Invalid File Handle: ")) +
           String(FileHandle));
  }
...
#ifdef SIMULATE_SD
  RenameFlashFile(String(CN_slashsdslash) + OldName,
                  String(CN_slashsdslash) + NewName);
#else
  LockSd();
...
//-----------------------------------------------------------------------------
bool c_FileMgr::handleFileUpload(const String &filename, size_t index,
  ...
  do // once
  {
    ...
    if (index != expectedIndex) {
      if (fsUploadFileHandle != INVALID_FILE_HANDLE) {
        logcon(String(F("ERROR: Expected index: ")) + String(expectedIndex) +
               F(" does not match actual index: ") + String(index));
...
#ifdef ARDUINO_ARCH_ESP32
      LOG_PORT.println(
          String("\033[Fprogress: ") + String(expectedIndex) +
          ", heap: " + String(heap_caps_get_largest_free_block(0x1800)));
#else
  ...
  if ((true == final) && (fsUploadFileHandle != INVALID_FILE_HANDLE)) {
    ...
    logcon(String(F("Upload File: '")) + fsUploadFileName + F("' Done (") +
           String(uploadTime) + F("s). Received: ") + String(expectedIndex) +
           F(" Bytes out of ") + String(totalLen) + F(" bytes. FileLen: ") +
           GetSdFileSize(filename));
...
void c_FileMgr::handleFileUploadNewFile(const String &filename) {
  ...
  if (!fsUploadFileName.isEmpty()) {
    logcon(String(F("Aborting Previous File Upload For: '")) +
           fsUploadFileName + String(F("'")));
    // DEBUG_FILE_HANDLE (fsUploadFileHandle);
    CloseSdFile(fsUploadFileHandle);
  }
  ...
  // DEBUG_V(String("fsUploadFileName: ") + String(fsUploadFileName));
  OpenSdFile(fsUploadFileName, FileMode::FileWrite, fsUploadFileHandle,
             -1 /*first access*/);
  int FileListIndex;
  if (-1 == (FileListIndex = FileListFindSdFileHandle(fsUploadFileHandle))) {
    logcon(String(F("WriteSdFileBuf::ERROR::Invalid File Handle: ")) +
           String(fsUploadFileHandle));
  } else {
    // DEBUG_V("Use the output buffer as a data buffer");
    FileList[FileListIndex].buffer.offset = 0;
    FileList[FileListIndex].buffer.size =
        min(uint32_t(OutputMgr.GetBufferSize() & ~(SD_BLOCK_SIZE - 1)),
            uint32_t(MAX_SD_BUFFER_SIZE));
    FileList[FileListIndex].buffer.DataBuffer = OutputMgr.GetBufferAddress();
    OutputMgr.PauseOutputs(true);
    InputMgr.SetOperationalState(false);
    OutputMgr.ClearBuffer();
    // DEBUG_V(String("Buffer Size: ") +
    // String(FileList[FileListIndex].buffer.size));
  }
...
//-----------------------------------------------------------------------------
bool c_FileMgr::SeekSdFile(const FileId &FileHandle, uint64_t position,
                           SeekMode Mode) {
  // DEBUG_START;
  ...
  {
    if (-1 == (FileListIndex = FileListFindSdFileHandle(FileHandle))) {
      logcon(String(F("SeekSdFile::ERROR::Invalid File Handle: ")) +
             String(FileHandle));
      break;
    }
    ...
    case SeekMode::SeekCur: {
      ...
      response =
          FileList[FileListIndex].fsFile.seek(CurrentPosition + position);
      break;
    }
    default: {
...
bool c_FileMgr::IsCompressed(String FileName) {
  ...
  Response = ((-1 != FileName.indexOf(F(".zip"))) ||
              (-1 != FileName.indexOf(F(".xlz"))));
