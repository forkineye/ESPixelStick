...
void c_InputMQTT::connectToMqtt()
{
    ...
    LOG_PORT.println (String(F ("Connecting to broker ")) + ip + ":" + String(port));
    mqtt.connect ();
...
void c_InputMQTT::disconnectFromMqtt ()
{
    ...
    LOG_PORT.println (String (F ("Disconnecting from broker")));
    mqtt.disconnect ();
...
void c_InputMQTT::onMqttConnect(bool sessionPresent)
{
    ...
    LOG_PORT.println (String (F ("Connected")));
...
void c_InputMQTT::onMqttDisconnect(AsyncMqttClientDisconnectReason reason)
{
    ...
    LOG_PORT.println (String(F ("Disconnected: ")) + String(DisconnectReasons[uint8_t(reason)]));
...
void c_InputMQTT::PlayFseq (JsonObject & JsonConfig)
{
    ...
    do // once
    {
        ...
        if (!FileIsPlayList && !FileIsStandalone)
        {
            ...
            LOG_PORT.println (String (F ("ERROR: Unsupported file type for File Play operation. File:'")) + FileName + "'");
            break;
        }
...
void c_InputMQTT::NetworkStateChanged (bool IsConnected, bool ReBootAllowed)
{
    ...
    else if (ReBootAllowed)
    {
        ...
        extern bool reboot;
        reboot = true;
        LOG_PORT.println (String (F ("Requesting reboot on loss of network connection.")));
    }
    else
    {
        onNetworkDisconnect ();
