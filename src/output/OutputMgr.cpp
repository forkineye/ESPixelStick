...
{
    // DEBUG_START;
    log (F ("--- WARNING: Creating a new Output Manager configuration Data set - Start ---"));
    ...
    log (F ("--- WARNING: Creating a new Output Manager configuration Data set - Done ---"));
    // DEBUG_END;
...
void c_OutputMgr::InstantiateNewOutputChannel (e_OutputChannelIds ChannelIndex, e_OutputType NewOutputChannelType, bool StartDriver)
{
    ...
    do // once
    {
        ...
        if (nullptr != pOutputChannelDrivers[ChannelIndex])
        {
            ...
            log (CN_stars + String (F (" Shutting Down '")) + DriverName + String (F ("' on Output: ")) + String (ChannelIndex) + " " + CN_stars);
        ...
        switch (NewOutputChannelType)
        {
            ...
            {
                // log (CN_stars + String (F (" Disabled output type for channel '")) + ChannelIndex + "'. **************");
            ...
            case e_OutputType::OutputType_DMX:
            {
                ...
                {
                    // log (CN_stars + String (F (" Starting DMX for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                // DEBUG_V ("");
                log (CN_stars + String (F (" Cannot Start DMX for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
                break;
            }
            ...
            case e_OutputType::OutputType_GECE:
            {
                if ((ChannelIndex >= OutputChannelId_UART_FIRST) && (ChannelIndex <= OutputChannelId_UART_LAST))
                {
                    // log (CN_stars + String (F (" Starting GECE for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                log (CN_stars + String (F (" Cannot Start GECE for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
            ...
            case e_OutputType::OutputType_Serial:
            {
                if (OM_IS_UART)
                {
                    // log (CN_stars + String (F (" Starting Generic Serial for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                log (CN_stars + String (F (" Cannot Start Generic Serial for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
            ...
            case e_OutputType::OutputType_Relay:
            {
                ...
                {
                    // log (CN_stars + String (F (" Starting RELAY for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                log (CN_stars + String (F (" Cannot Start RELAY for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
                break;
            }
            ...
            case e_OutputType::OutputType_Renard:
            {
                if (OM_IS_UART)
                {
                    // log (CN_stars + String (F (" Starting Renard for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                log (CN_stars + String (F (" Cannot Start Renard for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
                break;
            }
            ...
            case e_OutputType::OutputType_Servo_PCA9685:
            {
                if (ChannelIndex == OutputChannelId_Relay)
                {
                    // log (CN_stars + String (F (" Starting Servo PCA9685 for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                log (CN_stars + String (F (" Cannot Start Servo PCA9685 for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
                break;
            }
            ...
            case e_OutputType::OutputType_WS2811:
            {
                ...
                if (OM_IS_RMT)
                {
                    // log (CN_stars + String (F (" Starting WS2811 RMT for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                if (OM_IS_UART)
                {
                    // log (CN_stars + String (F (" Starting WS2811 UART for channel '")) + ChannelIndex + "'. " + CN_stars);
                    pOutputChannelDrivers[ChannelIndex] = new c_OutputWS2811Uart (ChannelIndex, dataPin, UartId, OutputType_WS2811);
                    // DEBUG_V ("");
                    break;
                }

                log (CN_stars + String (F (" Cannot Start WS2811 for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
            ...
            case e_OutputType::OutputType_TM1814:
            {
                ...
                if (OM_IS_RMT)
                {
                    // log (CN_stars + String (F (" Starting TM1814 RMT for channel '")) + ChannelIndex + "'. " + CN_stars);
                ...
                if (OM_IS_UART)
                {
                    // log (CN_stars + String (F (" Starting TM1814 UART for channel '")) + ChannelIndex + "'. " + CN_stars);
            ...
            case e_OutputType::OutputType_WS2801:
            {
                ...
                log (CN_stars + String (F (" Cannot Start WS2801 for channel '")) + ChannelIndex + "'. " + CN_stars);
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
            ...
            default:
            {
                log (CN_stars + String (F (" Unknown output type: '")) + String (NewOutputChannelType) + String(F(" for channel '")) + ChannelIndex + String(F("'. Using disabled. ")) + CN_stars );
                pOutputChannelDrivers[ChannelIndex] = new c_OutputDisabled (ChannelIndex, dataPin, UartId, OutputType_Disabled);
                // DEBUG_V ("");
                break;
            }
...
void c_OutputMgr::LoadConfig ()
{
    ...
    {
        log (F ("EEEE Error loading Output Manager Config File. EEEE"));
...
bool c_OutputMgr::ProcessJsonConfig (JsonObject& jsonConfig)
{
    ...
    do // once
    {
        ...
        {
            log (F ("No Output Interface Settings Found. Using Defaults"));
            break;
        }
        ...
        {
            log (F ("OutputMgr: Incorrect Version found. Using existing/default config."));
            // break;
        }
            ...
            ChannelIndex++)
        {
            ...
            {
                // if not, flag an error and stop processing
                log (String (F ("No Output Settings Found for Channel '")) + ChannelIndex + String (F ("'. Using Defaults")));
                break;
            }
            ...
            if (/*(ChannelType < uint32_t (OutputType_Start)) || */ (ChannelType >= uint32_t (OutputType_End)))
            {
                ...
                log (String (F ("Invalid Channel Type in config '")) + ChannelType + String (F ("'. Specified for channel '")) + ChannelIndex + "'. Disabling channel");
                InstantiateNewOutputChannel (e_OutputChannelIds (ChannelIndex), e_OutputType::OutputType_Disabled);
                continue;
            }
            // DEBUG_V ("");
...
void c_OutputMgr::SetConfig (const char * ConfigData)
{
    ...
    if (true == FileMgr.SaveConfigFile (ConfigFileName, ConfigData))
    {
        ...
        log (CN_stars + String (F (" Saved Output Manager Config File. ")) + CN_stars);
    } // end we got a config and it was good
    else
    {
        log (F ("EEEE Error Saving Output Manager Config File. EEEE"));
    }
...
void c_OutputMgr::UpdateDisplayBufferReferences (void)
{
    ...
    for (c_OutputCommon* pOutputChannel : pOutputChannelDrivers)
    {
        ...
        {
            log (String (F ("--- OutputMgr: ERROR: Too many output channels have been Requested: ")) + String (ChannelsNeeded));
            // DEBUG_V (String ("    ChannelsNeeded: ") + String (ChannelsNeeded));
            // DEBUG_V (String (" AvailableChannels: ") + String (AvailableChannels));
            // DEBUG_V (String ("ChannelsToAllocate: ") + String (ChannelsToAllocate));
        }
