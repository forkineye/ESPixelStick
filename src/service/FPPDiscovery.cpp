...
void c_FPPDiscovery::NetworkStateChanged (bool NewNetworkState)
{
    ...
    do // once
    {
        ...
        {
            LOG_PORT.println (String (F ("FAILED to subscribed to broadcast messages")));
            fail = true;
            break;
        }
        ...
        {
            LOG_PORT.println (String (F ("FAILED to subscribed to multicast messages")));
            fail = true;
            break;
        }
        ...
        if (!fail)
            LOG_PORT.println (String (F ("Listening on port ")) + String(FPP_DISCOVERY_PORT));
    ...
    do // once
    {
        ...
        if (path.startsWith (F ("/api/sequence/")) && AllowedToRemotePlayFiles())
        {
            ...
            if (seq.endsWith (F ("/meta")))
            {
                ...
                LOG_PORT.println (String (F ("Could not open: ")) + seq);
            }
        }
        request->send (404);
