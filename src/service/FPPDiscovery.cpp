    ...
    do // once
    {
        ...
        switch (action)
        {
            ...
            case 0x02: // Sync
            {
                ...
                {
                    // DEBUG_V ("Sync::Sync");
                    // LOG_PORT.println (String(float(millis()/1000.0)) + "," + String(InputFPPRemotePlayFile.GetLastFrameId()) + "," + String (seconds_elapsed) + "," + String (FrameId));
...
void c_FPPDiscovery::StartPlaying (String & FileName, uint32_t FrameId)
{
    ...
    do // once
    {
        ...
        LOG_PORT.println (String ("\nESP Sec") + "," + String ("ESP Frame ID") + "," + String ("FPP Seconds") + "," + String ("FPP Frame ID"));
