...
void fsm_PlayFile_state_PlayingFile::Poll (uint8_t* Buffer, size_t BufferSize)
{
    ...
    do // once
    {
        ...
        if (p_InputFPPRemotePlayFile->TotalNumberOfFramesInSequence <= CurrentFrame)
        {
            ...
            {
                NewLogToCon (String ("Replaying:: FileName:  '") + p_InputFPPRemotePlayFile->GetFileName () + "'");
        ...
        if (bytesRead != toRead)
        {
            ...
            {
                NewLogToCon (F ("File Playback Failed to read enough data"));
                Stop ();
            }
        }
...
void fsm_PlayFile_state_PlayingFile::Init (c_InputFPPRemotePlayFile* Parent)
{
    ...
    do // once
    {
        ...
        {
            NewLogToCon (String (F ("StartPlaying:: Could not open file: filename: '")) + p_InputFPPRemotePlayFile->PlayItemName + "'");
            Stop ();
            break;
        }
        ...
        {
            NewLogToCon (String (F ("StartPlaying:: Could not read FSEQ header: filename: '")) + p_InputFPPRemotePlayFile->PlayItemName + "'");
            Stop ();
            break;
        }
        // DEBUG_V ("");
        ...
        {
            NewLogToCon (String (F ("StartPlaying:: Could not start. ")) + p_InputFPPRemotePlayFile->PlayItemName + F (" is not a v2 uncompressed sequence"));
            Stop ();
            break;
        }
        // DEBUG_V ("");
        ...
        NewLogToCon (String (F ("Start Playing:: FileName:  '")) + p_InputFPPRemotePlayFile->PlayItemName + "'");
...
void fsm_PlayFile_state_PlayingFile::Stop (void)
{
    ...
    NewLogToCon (String (F ("Stop Playing:: FileName:  '")) + p_InputFPPRemotePlayFile->PlayItemName + "'");
    // DEBUG_V (String ("FileHandleForFileBeingPlayed: ") + String (p_InputFPPRemotePlayFile->FileHandleForFileBeingPlayed));
